...
const getAll = async (req: Request, res: Response) => {
  try {
    const courses = await db('course')
      ...
      .leftJoin(
        db
          .select('course_id', db.raw('ARRAY_AGG(DISTINCT student_id) AS students'))
          .from('student_course')
          .groupBy('course_id')
          .as('student_courses'),
  ...
  try {
    ...
    course.forEach((row) => {
      ...
      const lesson: Lesson = {
        ...
        lesson_resources: [{ lesson_resources: row.lesson_resources } as Resource],
      };
...
export { getAll, getCourseById, addNewCourse, startNewCourse, checkEnrollment };
